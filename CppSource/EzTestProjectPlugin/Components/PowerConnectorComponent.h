...
/// Both the previous and new value are sent, so that the different can be calculated.
/// This is useful in case a script has multiple connectors and needs to keep track of the total amount of power available.
class ezEventMsgSetPowerInput : public ezEventMessage
{
...
/// be further propagated as the 'input' on that object. This can go through many hops until the value reaches the final connector (if you build
